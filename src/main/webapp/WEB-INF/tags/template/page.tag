	...
	<div class="wizard-comtainer">
		...
		<div class="wizard">
			...
			<div class="resume">
				...
				<ul>
					<c:forEach items="${ongoingSetupState.commands}" var="command">
						<li><spring:message code="${command.getNameKey()}"/> <span class="item-list-price">${command.timeToExecute}</span></li>
					</c:forEach>
				</ul>
				<c:if test="${ongoingSetupState != null}">
						<footer>
							<h2>Time saved: ${ongoingSetupState.finalTime}</h2>
						</footer>
				</c:if>
			</div>
		</div>
